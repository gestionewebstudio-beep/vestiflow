...
| **D7**    | ⛔ Non «tutte fuori dal backup»: gli eventi accolti e non applicati devono essere recuperabili dopo un ripristino; prova di backup/ripristino; nessuna cancellazione per anzianità di pendenti o fallite; pulizia oltre 30 giorni **solo** per ricevute concluse, con deduplicazione e dati personali considerati esplicitamente.      | `shopify_webhook_receipts` **entra** nel backup (tutte le righe); `shopify_webhook_lanes` no (è una rivendicazione di processo, come il claim: si differisce/azzera nel ripristino). ⏸ **Dopo un ripristino le ricevute `in_coda`/`fallita` tornano come `sospesa_dopo_ripristino`**: recuperabili con «Riprova», non applicate da sole — il tenant ripristinato può non essere più collegato a quel negozio. Pulizia: comando esplicito, solo `elaborata`/`superata`/`scartata_*` più vecchie di 30 giorni; deduplicazione salva perché la finestra di riconsegna di Shopify è di 48 h ≪ 30 gg; il `payload` porta dati personali (clienti, ordini) e va via con la riga; i log non lo contengono mai                                                                                                                  |

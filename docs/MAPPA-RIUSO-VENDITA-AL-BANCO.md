...
| T9  | ✅ **CHIUSO 22/08/2026** — **Eliminazione**: i tre cancelli erano `FLOW_ONLY`, il gate di stato (`confirmed`) e l'assenza dai tipi che stornano. Tolti tutti e tre insieme: toglierne uno solo avrebbe eliminato il documento **senza restituire la merce**                                                                                                          | `documents.service.ts` · `delete()`                                                                                                                                                                                                 |
| T10 | ✅ **CHIUSO 22/08/2026** — la neutralizzazione passa dai motori comuni con `lines: []`: SCARICO per la Vendita, CARICO per il Reso. ⚠️ Il verso conta — un motore solo rimetterebbe in casa la merce di un reso eliminato, cioè il doppio                                                                                                                            | stesso posto                                                                                                                                                                                                                        |
| T11 | ✅ **CHIUSO 22/08/2026** — dei nove punti d'uso di `isStoreFlowDocumentType` ne sono cambiati **tre**: l'azione di riga, `canDelete` del Dettaglio e l'elenco della selezione multipla (ora vuoto). Gli altri sei restano: pagamento, annullamento (`11` A2: si elimina, non si annulla), duplica, e le rese del Dettaglio                                           | frontend                                                                                                                                                                                                                            |
| T12 | ✅ **CHIUSO 22/08/2026** — l'azione di riga diceva «Apri» con un commento «mai una modifica»: testo rimasto indietro rispetto a `14` §2. Ora dice «Apri / Modifica» come ogni altro tipo                                                                                                                                                                             | frontend elenchi                                                                                                                                                                                                                    |
...
✅ 14 · eliminazione FATTO il 22/08 — T9 · T10 · T11 · T12 chiusi. `DELETE
                   /documents/:id` accetta i due tipi in stato `confirmed` e
                   NEUTRALIZZA: movimenti tolti riga per riga e merce
                   restituita, col motore del verso giusto. Il Registro
                   Corrispettivi si aggiorna da sé — legge i documenti.
                   ⛔ L'annullamento resta escluso: `11` A2 dice «si elimina,
                   non si annulla», e un annullato occuperebbe un numero
                   senza dire niente.
...
| **A15** — default della spunta su una riga nuova                           | articolo che gestisce il magazzino → attiva; servizio → disattiva; sempre modificabile                                               | `VariantSummary.managesStock`                                                        | ✅ deciso il 21/08, A15 aggiornata                 |

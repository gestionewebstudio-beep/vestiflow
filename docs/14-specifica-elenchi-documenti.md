...
⭐ **E dal 21/08/2026 non esiste nemmeno un tetto di RISPOSTA** sugli elenchi: quante righe
l'API consegna non è limitato, e a contenerle è il **periodo** (§H14-bis). Per un momento ce n'era
uno da 2.000 — proprio il numero che questa riga nomina — ed è stato tolto: _«per il momento deve
essere semplice»_.
...
### ⛔ NESSUN tetto sulle righe — deciso il 21/08/2026

> _«Troppo complesso. Per il momento deve essere semplice: il riepilogo apre secondo le regole di
> default e non ancora con limiti. Per regolare il flusso, dove possibile, stiamo aprendo le
> maschere con i filtri applicati periodici.»_

⭐ **Il contenimento è il PERIODO**, non un numero di righe. Un elenco che si apre sugli ultimi
30 giorni è già delimitato da qualcosa che l'operatore capisce e può cambiare; un tetto sul
conteggio sarebbe una seconda regola — con un suo avviso, un suo campo nel contratto e un suo
caso limite — per un problema che nessuno ha ancora misurato.

⚠️ **Qui c'era un tetto da 2.000 righe con troncamento dichiarato nel `meta`**, introdotto il
20/08 come «scelta di sicurezza». È stato **tolto**: `UNPAGED_MAX_ROWS`, `unpagedResult`,
il campo `truncated` del contratto e i tre avvisi a schermo. Con `all` l'API consegna tutto il
risultato del filtro.

⭐ **E così cade anche la divergenza** che questa sezione registrava: non ci sono più due

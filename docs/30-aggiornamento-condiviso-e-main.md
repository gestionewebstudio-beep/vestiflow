...
| ------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **permesso `read_merchant_managed_fulfillment_orders` assente su Railway** → 2 notifiche non registrate | le spedizioni per riga e sede dai fulfillment order non arrivano finché manca. Proposta: aggiungerlo a `SHOPIFY_SCOPES` su Railway (ridistribuzione automatica), poi una nuova autorizzazione (Disconnetti → Connetti: gli abbinamenti sopravvivono, le sedi si riconoscono dalla coppia attiva) e registrazione delle notifiche mancanti. Con il via del proprietario |
| le 5 sedi portano ancora `shopify_sync_status = error` («non risulta più disponibile»)                  | scritto alle 10:50:57Z dalla sincronizzazione della riconnessione, quando erano scollegate; «collega» scrive solo l'id e non azzera il flag. Non blocca nulla (gli invii non leggono quello stato). Si azzera con «Sincronizza location» (risincronizzazione: `synced`, errore a `null`)                                                                               |

...
export class DocumentCounterpartyRefComponent {
  ...
  /** Chi può creare e riordinare i tipi documento del tenant. */
  private readonly puoConfigurareDocumenti = computed(() =>
    hasTenantPermission(this.auth.currentUser(), TenantPermission.DocumentsConfigure),
  );
  ...
  protected readonly options = computed<readonly SelectMenuOption[]>(() => {
    ...
    // La voce crea, riordina, disattiva ed elimina i tipi del tenant: senza il
    // permesso l'API la nega, e il comando risponderebbe 403 al primo clic. Chi
    // non configura i documenti sceglie soltanto fra quelli che esistono.
    //
    // Il controllo sta QUI perché qui è passato il gesto: viveva nelle maschere,
    // e il blocco condiviso della controparte (12/08/2026) se l'è portato
    // dietro. Nei chiamanti sarebbe ripetuto sette volte, e basterebbe
    // dimenticarlo una.
    if (!this.disabled() && this.puoConfigurareDocumenti()) {

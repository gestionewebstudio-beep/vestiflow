...
export class ShopifyAdminClient {
  constructor(
    ...
    private readonly http: ShopifyAdminHttpClient,
  ) {}
  ...
  /**
   * Il corpo di questo metodo vive ora in ShopifyAdminHttpClient, condiviso con il client
   * di sola lettura dei webhook: quel client deve poter parlare con Shopify senza ereditare
   * la capacita' di registrare e cancellare, e non ci riuscirebbe se il trasporto restasse
   * qui dentro.
   */
  private request<T>(
    shopDomain: string,
    ...
    return this.http.request<T>(shopDomain, accessToken, path, init);
  }

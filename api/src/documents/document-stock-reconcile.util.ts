...
/**
 * Riconcilia giacenze dopo modifica documento confermato.
 * Se la location cambia: storno completo sulla vecchia, carico sulla nuova.
 */
export async function reconcileDocumentStockLoad(
  ...
  if (params.oldLocationId !== params.newLocationId) {
    for (const entry of oldMap.values()) {
      await applyStockUnload(tx, {
        ...
        reason: `${reasonBase}: cambio location (storno)`,
        externalRef,
        actor: params.actor,
      });
    ...
    for (const entry of newMap.values()) {
      await applyStockLoad(tx, {
        ...
        quantity: entry.quantity,
        reason: `${reasonBase}: cambio location (carico)`,
        externalRef,
        actor: params.actor,
      });
      deltas.push({ sku: entry.sku, delta: entry.quantity });
  ...
  for (const variantId of allVariantIds) {
    ...
    if (delta > 0) {
      await applyStockLoad(tx, {
        ...
        locationId: params.newLocationId,
        quantity: delta,
        reason: `${reasonBase}: rettifica carico +${delta}`,
        externalRef,
        actor: params.actor,
      });
    } else {

...
describe('isSupabaseOwnerEmailInviteEnabled', () => {
  it('false se variabile assente o diversa da true', () => {
    ...
    expect(
      isSupabaseOwnerEmailInviteEnabled(createConfig({ SUPABASE_OWNER_EMAIL_INVITE: 'false' })),
    ).toBe(false);
  });

  it('true solo con SUPABASE_OWNER_EMAIL_INVITE=true', () => {
    expect(
      isSupabaseOwnerEmailInviteEnabled(createConfig({ SUPABASE_OWNER_EMAIL_INVITE: 'true' })),
    ).toBe(true);
    expect(
      isSupabaseOwnerEmailInviteEnabled(createConfig({ SUPABASE_OWNER_EMAIL_INVITE: 'TRUE' })),
    ).toBe(true);

...
import type { Prisma, TenantUserAuditAction } from '@prisma/client';
...
/**
 * Meccanica condivisa del ciclo di vita utenti tenant (credenziali Supabase,
 * sedi, permessi, transazioni). Le REGOLE DI ACCESSO stanno nei chiamanti:
 * l'area admin piattaforma e l'API del titolare applicano ciascuna i propri
 * vincoli PRIMA di delegare qui.
 */
@Injectable()
export class TenantUsersCoreService {
  private readonly logger = new Logger(TenantUsersCoreService.name);
  ...
  async requireUser(
    tenantId: string,
    userId: string,
  ): Promise<{ readonly id: string; readonly role: UserRole; readonly email: string }> {
  ...
  async createUser(
    ...
    try {
      const user = await this.prisma.$transaction(async (tx) => {
        const created = await tx.user.create({
          data: {
            ...
            defaultLocationId,
            permissions,
            // La password iniziale la conosce chi crea l'account: al primo
            // accesso l'app chiede di cambiarla.
      ...
      const created = this.toDto(user);
      await this.writeAudit(tenantId, actor, 'created', created, tenantUserCreationDetails(this.toAuditSnapshot(created)));
      return created;
    } catch (error) {
  ...
  async updateUser(
    ...
    const updated = this.toDto(user);
    const diff = diffTenantUserSnapshots(beforeSnapshot, this.toAuditSnapshot(updated));
    if (Object.keys(diff).length > 0) {
      await this.writeAudit(tenantId, actor, 'updated', updated, diff);
    }
    return updated;
  }
  ...
  async deleteUser(
    ...
    await this.writeAudit(tenantId, actor, 'deleted', existing, {
      role: { before: existing.role, after: null },
    });
  ...
  /**
   * Scrive la riga di audit DOPO l'operazione riuscita: un errore qui non deve
   * annullare una modifica già avvenuta (le credenziali Supabase, ad esempio,
   * sono già state create o rimosse) — si logga e si prosegue. Senza attore
   * (chiamate interne o di test) non si scrive nulla.
   */
  private async writeAudit(
    tenantId: string,
    actor: TenantUserActionActor | undefined,
    action: TenantUserAuditAction,
    target: { readonly id: string | null; readonly email: string },
    details: Record<string, { readonly before: unknown; readonly after: unknown }>,
  ): Promise<void> {
    if (!actor) {
      return;
    }
    try {
      await this.prisma.tenantUserAuditLog.create({
        data: {
          tenantId,
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(`Audit utenti non scritto (${action} su ${target.email}): ${message}`);
    }
  }

  private toAuditSnapshot(user: TenantUserDto): TenantUserAuditSnapshot {
    return {

...
describe('TenantUsersCoreService', () => {
  ...
  let prisma: {
    ...
    tenantUserAuditLog: { create: ReturnType<typeof vi.fn> };
  ...
  let service: TenantUsersCoreService;
  ...
  beforeEach(() => {
    prisma = {
      ...
      supportSession: { deleteMany: vi.fn().mockResolvedValue({ count: 0 }) },
      tenantUserAuditLog: { create: vi.fn().mockResolvedValue({ id: 'audit-1' }) },
    ...
    service = new TenantUsersCoreService(
  ...
  // ── Audit e password iniziale ──────────────────────────────────────

  it('createUser marca la password iniziale come da cambiare e scrive l’audit', async () => {
    prisma.user.findFirst.mockResolvedValueOnce(null); // email univoca
    ...
    expect(prisma.tenantUserAuditLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          tenantId,
          action: 'created',
          actorUserId: 'user-owner',
          actorEmail: 'owner@test.it',
          targetEmail: 'clerk@test.it',
        }),
      }),
    );
  });

  it('updateUser scrive l’audit con il solo diff dei campi cambiati', async () => {
    ...
    await service.updateUser(tenantId, 'user-clerk', { isActive: false }, actor);

    expect(prisma.tenantUserAuditLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          action: 'updated',
          details: { isActive: { before: true, after: false } },
        }),
      }),
    );
  });

  it('updateUser senza modifiche reali non scrive audit', async () => {
    const row = {
      id: 'user-clerk',
      tenantId,
      authUserId: 'auth-clerk',
    ...
    await service.updateUser(tenantId, 'user-clerk', { displayName: 'Commesso' }, actor);

    expect(prisma.tenantUserAuditLog.create).not.toHaveBeenCalled();
  });

  it('senza attore (chiamate interne) non scrive audit', async () => {
    prisma.user.findFirst.mockResolvedValue({
      id: 'user-clerk',
      authUserId: 'auth-clerk',
      email: 'clerk@test.it',
    ...
    await service.deleteUser(tenantId, 'user-clerk');

    expect(prisma.tenantUserAuditLog.create).not.toHaveBeenCalled();
  });

  it('deleteUser scrive l’audit di eliminazione', async () => {
    prisma.user.findFirst.mockResolvedValue({
      id: 'user-clerk',
      authUserId: 'auth-clerk',
      email: 'clerk@test.it',
  ...
  it('un errore di scrittura audit non fa fallire l’operazione', async () => {
    prisma.tenantUserAuditLog.create.mockRejectedValue(new Error('db down'));
    prisma.user.findFirst.mockResolvedValue({
      id: 'user-clerk',
      authUserId: 'auth-clerk',
      email: 'clerk@test.it',
    ...
    await expect(service.deleteUser(tenantId, 'user-clerk', actor)).resolves.toBeUndefined();
    expect(prisma.user.delete).toHaveBeenCalled();
  });

  // ── Meccanica eliminazione (già coperta via facciata admin, ora qui) ─
